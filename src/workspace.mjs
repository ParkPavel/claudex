...
export async function syncWorkspace(ws) {
  ...
  // Native definitions follow the installation, not the shipped table. A role
  // assigned to the other provider is written as that provider's agent, with the
  // model and effort actually chosen: otherwise a managed job and an interactive
  // session disagree about who the architect is, and nothing says which is right.
